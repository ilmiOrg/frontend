...
class EmailService {
  ...
  async sendApplicationEmail(applicationData) {
    const formData = {
      ...
      message:
        applicationData.personalStatement || "No personal statement provided",
    };
  ...
  async sendScholarshipEmail(scholarshipData) {
    const formData = {
      ...
      message:
        scholarshipData.inquiryMessage ||
        "Interested in scholarship information",
    };
  ...
  formatTelegramMessage(formData, formType) {
    ...
    let message = `${emoji} <b>New ${formType
      .replace("-", " ")
      .toUpperCase()} Submission</b>\n\n`;
